...
DEVNAME=${DEVNAME:=$1}
if [[ -z $DEVNAME ]]; then
  echo "Invalid device name: $DEVNAME" >> $LOGFILE
  exit 1
fi
...
if [[ -z $ID_BUS || -z $ID_FS_TYPE || -z $ID_FS_UUID_ENC || -z $ID_FS_LABEL_ENC ]]; then
  echo "Could not get device information: $DEVNAME" >> $LOGFILE
  exit 1
fi
...
if ! grep -qw $ID_FS_TYPE /proc/filesystems; then
  echo "File system not supported: $ID_FS_TYPE" >> $LOGFILE
  exit 1
fi

# Mount device
if findmnt -rno SOURCE,TARGET $DEVNAME >/dev/null; then
    echo "Device $DEVNAME is already mounted!" >> $LOGFILE
else
    echo "Mounting - Source: $DEVNAME - Destination: $MOUNT_POINT" >> $LOGFILE
    mkdir -p $MOUNT_POINT
